...
TEMPLATE_TEST_CASE("TRAP::Math::Vec4", "[math][vec][vec4]", i8, i16, i32, i64, u8, u16, u32, u64, f32, f64)
{
    ...
    SECTION("fmt::format")
    {
        STATIC_REQUIRE(fmt::is_formattable<Vec4>::value);
        static constexpr Vec4 v(TestType(5), TestType(10), TestType(15), TestType(20));
    ...
    SECTION("Structured bindings")
    {
        ...
        REQUIRE(z == TestType(3));
        REQUIRE(w == TestType(4));

        Vec4 v2(TestType(1), TestType(2), TestType(3), TestType(4));
        auto& [x2, y2, z2, w2] = v2;
        REQUIRE(x2 == TestType(1));
        REQUIRE(y2 == TestType(2));
        REQUIRE(z2 == TestType(3));
        REQUIRE(w2 == TestType(4));
    }
