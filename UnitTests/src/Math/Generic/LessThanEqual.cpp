...
TEMPLATE_TEST_CASE("TRAP::Math::LessThanEqual()", "[math][generic][lessthanequal][scalar]", i8, i16, i32, i64, u8, u16, u32, u64, f32, f64)
{
    SECTION("Normal cases")
    {
        ...
        STATIC_REQUIRE(!TRAP::Math::LessThanEqual(A, B));
        STATIC_REQUIRE( TRAP::Math::LessThanEqual(B, A));
        STATIC_REQUIRE( TRAP::Math::LessThanEqual(A, A));
        STATIC_REQUIRE( TRAP::Math::LessThanEqual(B, B));

        if constexpr(std::floating_point<TestType> || std::signed_integral<TestType>)
        {
            static constexpr TestType C(TestType(-1.0f));

            STATIC_REQUIRE(!TRAP::Math::LessThanEqual(B, C));
            STATIC_REQUIRE( TRAP::Math::LessThanEqual(C, B));
            STATIC_REQUIRE( TRAP::Math::LessThanEqual(C, C));
        }
    }
...
TEMPLATE_TEST_CASE("TRAP::Math::LessThanEqual()", "[math][generic][lessthanequal][vec]",
                   ...
                   TRAP::Math::Vec4ui8, TRAP::Math::Vec4ui16, TRAP::Math::Vec4ui32, TRAP::Math::Vec4ui64, TRAP::Math::Vec4f, TRAP::Math::Vec4d)
{
    ...
    STATIC_REQUIRE(!TRAP::Math::All(TRAP::Math::LessThanEqual(A, B)));
    STATIC_REQUIRE( TRAP::Math::All(TRAP::Math::LessThanEqual(B, A)));
    STATIC_REQUIRE( TRAP::Math::All(TRAP::Math::LessThanEqual(A, A)));
    STATIC_REQUIRE( TRAP::Math::All(TRAP::Math::LessThanEqual(B, B)));

    if constexpr(std::floating_point<Scalar> || std::signed_integral<Scalar>)
    {
        static constexpr TestType C(Vec4Scalar(-2, -3, -4, -5));

        STATIC_REQUIRE(!TRAP::Math::All(TRAP::Math::LessThanEqual(A, C)));
        STATIC_REQUIRE( TRAP::Math::All(TRAP::Math::LessThanEqual(C, A)));
        STATIC_REQUIRE( TRAP::Math::All(TRAP::Math::LessThanEqual(C, C)));
    }
}

TEMPLATE_TEST_CASE("TRAP::Math::LessThanEqual()", "[math][generic][lessthanequal][quat]", TRAP::Math::Quatf, TRAP::Math::Quatd)
{
