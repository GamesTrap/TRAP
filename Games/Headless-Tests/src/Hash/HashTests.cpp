...
{
    constexpr std::array<uint8_t, 4> emptyHash = {0x00, 0x00, 0x00, 0x01};
    assert(TRAP::Utils::Hash::Adler32("") == emptyHash);

    constexpr std::array<uint8_t, 4> test1Hash = {0x36, 0xC4, 0x05, 0xFE};
    assert(TRAP::Utils::Hash::Adler32("Eagles are great!") == test1Hash);

    constexpr std::array<uint8_t, 4> test2Hash = {0x04, 0x0F, 0x0F, 0xC1};
...
{
    constexpr std::array<uint8_t, 4> emptyHash = {0x00, 0x00, 0x00, 0x00};
    assert(TRAP::Utils::Hash::CRC32("") == emptyHash);

    constexpr std::array<uint8_t, 4> foxHash = {0x41, 0x4F, 0xA3, 0x39};
    assert(TRAP::Utils::Hash::CRC32("The quick brown fox jumps over the lazy dog") == foxHash);

    constexpr std::array<uint8_t, 4> testData1Hash{0x9B, 0xD3, 0x66, 0xAE};
    constexpr std::array<uint8_t, 33> testData1 =
    {
...
{
    constexpr std::array<uint8_t, 32> abcHash{0xBA, 0x78, 0x16, 0xBF, 0x8F, 0x01, 0xCF, 0xEA, 0x41, 0x41, 0x40,
                                              0xDE, 0x5D, 0xAE, 0x22, 0x23, 0xB0, 0x03, 0x61, 0xA3, 0x96, 0x17,
                                              0x7A, 0x9C, 0xB4, 0x10, 0xFF, 0x61, 0xF2, 0x00, 0x15, 0xAD};
    assert(TRAP::Utils::Hash::SHA2_256("abc") == abcHash);
    constexpr std::array<uint8_t, 32> emptyHash{0xE3, 0xB0, 0xC4, 0x42, 0x98, 0xFC, 0x1C, 0x14, 0x9A, 0xFB, 0xF4,
                                               0xC8, 0x99, 0x6F, 0xB9, 0x24, 0x27, 0xAE, 0x41, 0xE4, 0x64, 0x9B,
                                               0x93, 0x4C, 0xA4, 0x95, 0x99, 0x1B, 0x78, 0x52, 0xB8, 0x55};
    assert(TRAP::Utils::Hash::SHA2_256("") == emptyHash);
    constexpr std::array<uint8_t, 32> longHash{0x24, 0x8D, 0x6A, 0x61, 0xD2, 0x06, 0x38, 0xB8, 0xE5, 0xC0, 0x26,
                                               0x93, 0x0C, 0x3E, 0x60, 0x39, 0xA3, 0x3C, 0xE4, 0x59, 0x64, 0xFF,
                                               0x21, 0x67, 0xF6, 0xEC, 0xED, 0xD4, 0x19, 0xDB, 0x06, 0xC1};
    assert(TRAP::Utils::Hash::SHA2_256("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq") == longHash);
    constexpr std::array<uint8_t, 32> longerHash{0xCF, 0x5B, 0x16, 0xA7, 0x78, 0xAF, 0x83, 0x80, 0x03, 0x6C, 0xE5,
    ...
    constexpr std::array<uint8_t, 32> aHash{0xCD, 0xC7, 0x6E, 0x5C, 0x99, 0x14, 0xFB, 0x92, 0x81, 0xA1, 0xC7,
                                            0xE2, 0x84, 0xD7, 0x3E, 0x67, 0xF1, 0x80, 0x9A, 0x48, 0xA4, 0x97,
                                            0x20, 0x0E, 0x04, 0x6D, 0x39, 0xCC, 0xC7, 0x11, 0x2C, 0xD0};
    const std::string aTest(1'000'000, 'a');
    assert(TRAP::Utils::Hash::SHA2_256(aTest) == aHash);

    constexpr std::array<uint8_t, 32> extremeHash{0x50, 0xE7, 0x2A, 0x0E, 0x26, 0x44, 0x2F, 0xE2, 0x55, 0x2D, 0xC3,
...
{
    constexpr std::array<uint8_t, 64> abcHash{0xDD, 0xAF, 0x35, 0xA1, 0x93, 0x61, 0x7A, 0xBA, 0xCC, 0x41, 0x73,
    ...
    constexpr std::array<uint8_t, 64> emptyHash{0xCF, 0x83, 0xE1, 0x35, 0x7E, 0xEF, 0xB8, 0xBD, 0xF1, 0x54, 0x28,
    ...
    constexpr std::array<uint8_t, 64> longHash{0x20, 0x4A, 0x8F, 0xC6, 0xDD, 0xA8, 0x2F, 0x0A, 0x0C, 0xED, 0x7B,
    ...
    constexpr std::array<uint8_t, 64> longerHash{0x8E, 0x95, 0x9B, 0x75, 0xDA, 0xE3, 0x13, 0xDA, 0x8C, 0xF4, 0xF7,
    ...
    constexpr std::array<uint8_t, 64> aHash{0xE7, 0x18, 0x48, 0x3D, 0x0C, 0xE7, 0x69, 0x64, 0x4E, 0x2E, 0x42,
    ...
    constexpr std::array<uint8_t, 64> extremeHash{0xB4, 0x7C, 0x93, 0x34, 0x21, 0xEA, 0x2D, 0xB1, 0x49, 0xAD, 0x6E,
...
{
    constexpr std::array<uint8_t, 32> abcHash{0x3A, 0x98, 0x5D, 0xA7, 0x4F, 0xE2, 0x25, 0xB2, 0x04, 0x5C, 0x17,
    ...
    constexpr std::array<uint8_t, 32> emptyHash{0xA7, 0xFF, 0xC6, 0xF8, 0xBF, 0x1E, 0xD7, 0x66, 0x51, 0xC1, 0x47,
    ...
    constexpr std::array<uint8_t, 32> longHash{0x41, 0xC0, 0xDB, 0xA2, 0xA9, 0xD6, 0x24, 0x08, 0x49, 0x10, 0x03,
    ...
    constexpr std::array<uint8_t, 32> longerHash{0x91, 0x6F, 0x60, 0x61, 0xFE, 0x87, 0x97, 0x41, 0xCA, 0x64, 0x69,
    ...
    constexpr std::array<uint8_t, 32> aHash{0x5C, 0x88, 0x75, 0xAE, 0x47, 0x4A, 0x36, 0x34, 0xBA, 0x4F, 0xD5,
    ...
    constexpr std::array<uint8_t, 32> extremeHash{0xEC, 0xBB, 0xC4, 0x2C, 0xBF, 0x29, 0x66, 0x03, 0xAC, 0xB2, 0xC6,
...
{
    constexpr std::array<uint8_t, 64> abcHash{0xB7, 0x51, 0x85, 0x0B, 0x1A, 0x57, 0x16, 0x8A, 0x56, 0x93, 0xCD,
    ...
    constexpr std::array<uint8_t, 64> emptyHash{0xA6, 0x9F, 0x73, 0xCC, 0xA2, 0x3A, 0x9A, 0xC5, 0xC8, 0xB5, 0x67,
    ...
    constexpr std::array<uint8_t, 64> longHash{0x04, 0xA3, 0x71, 0xE8, 0x4E, 0xCF, 0xB5, 0xB8, 0xB7, 0x7C, 0xB4,
    ...
    constexpr std::array<uint8_t, 64> longerHash{0xAF, 0xEB, 0xB2, 0xEF, 0x54, 0x2E, 0x65, 0x79, 0xC5, 0x0C, 0xAD,
    ...
    constexpr std::array<uint8_t, 64> aHash{0x3C, 0x3A, 0x87, 0x6D, 0xA1, 0x40, 0x34, 0xAB, 0x60, 0x62, 0x7C, 0x07,
    ...
    constexpr std::array<uint8_t, 64> extremeHash{0x23, 0x5F, 0xFD, 0x53, 0x50, 0x4E, 0xF8, 0x36, 0xA1, 0x34, 0x2B,
