...
{
	static_assert(major < 1024u, "Major version number must be less than 1024.");
	static_assert(minor < 1024u, "Minor version number must be less than 1024.");
	static_assert(patch < 4096u, "Patch version number must be less than 4096.");
...
template<u32 major, u32 minor, u32 patch>
struct fmt::formatter<SemanticVersion<major, minor, patch>>
{
    ...
    fmt::format_context::iterator format(const SemanticVersion<major, minor, patch>& semVer, fmt::format_context& ctx) const
    {
        auto out = ctx.out();
...
constexpr SemanticVersion<0u, 3u, 1u> CONVERTTOSPIRV_VERSION{};
