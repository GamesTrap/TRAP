...
TRAP::LayerStack::~LayerStack()
{
	...
	{
		TRAP_DEBUG("[LayerStack] Destroying Layer: ", layer->GetName());
		layer->OnDetach();
		layer.reset();
	}
}
...
{
	TRAP_DEBUG("[LayerStack] Pushing Layer: ", layer->GetName());
	layer->OnAttach();
...
{
	TRAP_DEBUG("[LayerStack] Pushing Overlay: ", overlay->GetName());
	overlay->OnAttach();
...
void TRAP::LayerStack::PopOverlay(const std::unique_ptr<Layer>& overlay)
{
	...
	if (it != m_layers.end())
	{
		TRAP_DEBUG("[LayerStack] Destroying Overlay: ", overlay->GetName());
		overlay->OnDetach();
		m_layers.erase(it);
	}
}