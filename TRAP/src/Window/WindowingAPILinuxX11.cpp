...
void TRAP::INTERNAL::WindowingAPI::UpdateNormalHints(InternalWindow* window, const int32_t width,
                                                     const int32_t height)
{
	XSizeHints* hints = s_Data.X11.XLIB.AllocSizeHints();

	long supplied;
	s_Data.X11.XLIB.GetWMNormalHints(s_Data.X11.display, window->Handle, hints, &supplied);
	...
	s_Data.X11.XLIB.SetWMNormalHints(s_Data.X11.display, window->Handle, hints);
...
bool TRAP::INTERNAL::WindowingAPI::WaitForVisibilityNotify(InternalWindow* window)
{
	...
	while(!s_Data.X11.XLIB.CheckTypedWindowEvent(s_Data.X11.display, window->Handle, VisibilityNotify, &dummy))
	{
...
void TRAP::INTERNAL::WindowingAPI::UpdateWindowMode(InternalWindow* window)
{
	if(window->Monitor)
	{
		...
		if(s_Data.X11.NET_WM_STATE && s_Data.X11.NET_WM_STATE_FULLSCREEN)
		{
			SendEventToWM(window,
		...
		else
		{
			...
			s_Data.X11.XLIB.ChangeWindowAttributes(s_Data.X11.display, window->Handle, CWOverrideRedirect,
			                                       &attributes);
		...
		if(!window->Transparent)
		{
			...
			s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle,
	...
	else
	{
		if(s_Data.X11.Xinerama.Available && s_Data.X11.NET_WM_FULLSCREEN_MONITORS)
		{
			s_Data.X11.XLIB.DeleteProperty(s_Data.X11.display, window->Handle,
			                               s_Data.X11.NET_WM_FULLSCREEN_MONITORS);
		}
		...
		else
		{
			...
			s_Data.X11.XLIB.ChangeWindowAttributes(s_Data.X11.display, window->Handle,
		...
		if(!window->Transparent)
			s_Data.X11.XLIB.DeleteProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_BYPASS_COMPOSITOR);
	}
}
...
void TRAP::INTERNAL::WindowingAPI::GetSystemContentScale(float& xScale, float& yScale)
{
	...
	const char* rms = s_Data.X11.XLIB.ResourceManagerString(s_Data.X11.display);
	if(rms)
	{
		...
		if(db)
		{
			...
			if(s_Data.X11.XRM.GetResource(db, "Xft.dpi", "Xft.Dpi", &type, &value))
			{
...
bool TRAP::INTERNAL::WindowingAPI::InitExtensions()
{
	...
	if(s_Data.X11.RandR.Handle)
	{
		...
		if(s_Data.X11.RandR.QueryExtension(s_Data.X11.display, &s_Data.X11.RandR.EventBase, &s_Data.X11.RandR.ErrorBase))
		{
			if(s_Data.X11.RandR.QueryVersion(s_Data.X11.display, &s_Data.X11.RandR.Major, &s_Data.X11.RandR.Minor))
			{
	...
	{
		XRRScreenResources* sr = s_Data.X11.RandR.GetScreenResourcesCurrent(s_Data.X11.display, s_Data.X11.Root);
	...
	if(s_Data.X11.Xinerama.Handle)
	{
		...
		if(s_Data.X11.Xinerama.QueryExtension(s_Data.X11.display, &s_Data.X11.Xinerama.Major, &s_Data.X11.Xinerama.Minor))
		{
			if(s_Data.X11.Xinerama.IsActive(s_Data.X11.display))
	...
	s_Data.X11.XKB.Available = s_Data.X11.XKB.QueryExtension(s_Data.X11.display,
											                 ...
											                 &s_Data.X11.XKB.Minor);

	if(s_Data.X11.XKB.Available)
	{
		int32_t supported = 0;

		if(s_Data.X11.XKB.SetDetectableAutoRepeat(s_Data.X11.display, 1, &supported))
		{
			if(supported)
	...
	if(s_Data.X11.XRender.Handle)
	{
		...
		if(s_Data.X11.XRender.QueryExtension(s_Data.X11.display, &s_Data.X11.XRender.ErrorBase, &s_Data.X11.XRender.EventBase))
		{
			if(s_Data.X11.XRender.QueryVersion(s_Data.X11.display, &s_Data.X11.XRender.Major, &s_Data.X11.XRender.Minor))
	...
	if(s_Data.X11.XShape.Handle)
	{
		...
		if(s_Data.X11.XShape.QueryExtension(s_Data.X11.display, &s_Data.X11.XShape.ErrorBase, &s_Data.X11.XShape.EventBase))
		{
			if(s_Data.X11.XShape.QueryVersion(s_Data.X11.display, &s_Data.X11.XShape.Major, &s_Data.X11.XShape.Minor))
...
void TRAP::INTERNAL::WindowingAPI::DetectEWMH()
{
	...
	s_Data.X11.NET_WM_STATE                   = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_STATE");
    s_Data.X11.NET_WM_STATE_ABOVE             = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_STATE_ABOVE");
    s_Data.X11.NET_WM_STATE_FULLSCREEN        = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_STATE_FULLSCREEN");
    s_Data.X11.NET_WM_STATE_MAXIMIZED_VERT    = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_STATE_MAXIMIZED_VERT");
    s_Data.X11.NET_WM_STATE_MAXIMIZED_HORZ    = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_STATE_MAXIMIZED_HORZ");
    s_Data.X11.NET_WM_STATE_DEMANDS_ATTENTION = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_STATE_DEMANDS_ATTENTION");
    s_Data.X11.NET_WM_FULLSCREEN_MONITORS     = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_FULLSCREEN_MONITORS");
    s_Data.X11.NET_WM_WINDOW_TYPE             = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_WINDOW_TYPE");
    s_Data.X11.NET_WM_WINDOW_TYPE_NORMAL      = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WM_WINDOW_TYPE_NORMAL");
    s_Data.X11.NET_WORKAREA                   = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_WORKAREA");
    s_Data.X11.NET_CURRENT_DESKTOP            = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_CURRENT_DESKTOP");
    s_Data.X11.NET_ACTIVE_WINDOW              = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_ACTIVE_WINDOW");
    s_Data.X11.NET_FRAME_EXTENTS              = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_FRAME_EXTENTS");
    s_Data.X11.NET_REQUEST_FRAME_EXTENTS      = GetAtomIfSupported(supportedAtoms, atomCount, "_NET_REQUEST_FRAME_EXTENTS");
...
{
	TRAP_ASSERT(s_Data.X11.PrevErrorHandler == nullptr);
...
void TRAP::INTERNAL::WindowingAPI::PollMonitorsX11()
{
	...
	for(int32_t i = 0; i < sr->noutput; i++)
	{
		...
		XRROutputInfo* oi = s_Data.X11.RandR.GetOutputInfo(s_Data.X11.display, sr, sr->outputs[i]);
		...
		XRRCrtcInfo* ci = s_Data.X11.RandR.GetCrtcInfo(s_Data.X11.display, sr, oi->crtc);
...
Atom TRAP::INTERNAL::WindowingAPI::WriteTargetToProperty(const XSelectionRequestEvent* request)
{
	...
	if(request->target == s_Data.X11.TARGETS)
	{
		...
		std::array<Atom, 4> targets{s_Data.X11.TARGETS, s_Data.X11.MULTIPLE, s_Data.X11.UTF8_STRING, XA_STRING};
...
void TRAP::INTERNAL::WindowingAPI::PushSelectionToManagerX11()
{
	...
	while(true)
	{
		...
		while(s_Data.X11.XLIB.CheckIfEvent(s_Data.X11.display, &event, IsSelectionEvent, nullptr))
		{
			switch(event.type)
			{
...
void TRAP::INTERNAL::WindowingAPI::CreateInputContextX11(InternalWindow* window)
{
	...
	window->IC = s_Data.X11.XLIB.CreateIC(s_Data.X11.IM,
		                                  XNInputStyle,
	...
	s_Data.X11.XLIB.GetWindowAttributes(s_Data.X11.display, window->Handle, &attribs);

	uint64_t filter = 0;
	if(s_Data.X11.XLIB.GetICValues(window->IC, XNFilterEvents, &filter, nullptr) == nullptr)
		s_Data.X11.XLIB.SelectInput(s_Data.X11.display, window->Handle, static_cast<int64_t>(attribs.your_event_mask | filter));
}
...
bool TRAP::INTERNAL::WindowingAPI::IsVisualTransparentX11(Visual* visual)
{
	...
	const XRenderPictFormat* pf = s_Data.X11.XRender.FindVisualFormat(s_Data.X11.display, visual);
                                                      ...
                                                      Visual* visual, int32_t depth)
{
	...
	window->colormap = s_Data.X11.XLIB.CreateColormap(s_Data.X11.display, s_Data.X11.Root, visual, AllocNone);
	...
	window->Parent = s_Data.X11.Root;
	window->Handle = s_Data.X11.XLIB.CreateWindow(s_Data.X11.display, s_Data.X11.Root,
	                                              0, 0,
								                  width, height,
								                  0,
								                  depth,
	...
	s_Data.X11.XLIB.SaveContext(s_Data.X11.display, window->Handle, s_Data.X11.Context, reinterpret_cast<XPointer>(window));
	...
	if(s_Data.X11.NET_WM_STATE && !window->Monitor)
	{
		...
		{
			if(s_Data.X11.NET_WM_STATE_ABOVE)
				states[count++] = s_Data.X11.NET_WM_STATE_ABOVE;
		}

		if(WNDConfig.Maximized)
		{
			if(s_Data.X11.NET_WM_STATE_MAXIMIZED_VERT &&
		...
		if(count)
		{
			s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_STATE, XA_ATOM, 32,
	...
	//Declare the WM protocols supported by TRAP
	{
		...
		s_Data.X11.XLIB.SetWMProtocols(s_Data.X11.display, window->Handle, protocols.data(), protocols.size());
	}

	//Declare our PID
	{
		int64_t pid = getpid();
		s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_PID, XA_CARDINAL, 32,
	...
	if(s_Data.X11.NET_WM_WINDOW_TYPE && s_Data.X11.NET_WM_WINDOW_TYPE_NORMAL)
	{
		...
		s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_WINDOW_TYPE, XA_ATOM, 32,
		                               PropModeReplace, reinterpret_cast<uint8_t*>(&type), 1);
	}

	//Set ICCCM WM_HINTS property
	{
		XWMHints* hints = s_Data.X11.XLIB.AllocWMHints();
		if(!hints)
		{
		...
		s_Data.X11.XLIB.SetWMHints(s_Data.X11.display, window->Handle, hints);
		s_Data.X11.XLIB.Free(hints);
	}
	...
	{
		XClassHint* hint = s_Data.X11.XLIB.AllocClassHint();
		...
		s_Data.X11.XLIB.SetClassHint(s_Data.X11.display, window->Handle, hint);
                                                     ...
                                                     const int32_t yHotSpot)
{
	...
	XcursorImage* native = s_Data.X11.XCursor.ImageCreate(static_cast<int32_t>(image->GetWidth()),
...
int32_t TRAP::INTERNAL::WindowingAPI::GetWindowState(const InternalWindow* window)
{
	...
	if(GetWindowPropertyX11(window->Handle, s_Data.X11.WM_STATE, s_Data.X11.WM_STATE,
...
std::string TRAP::INTERNAL::WindowingAPI::GetSelectionString(const Atom selection)
{
	...
	if(selection == s_Data.X11.PRIMARY)
		selectionString = &s_Data.X11.PrimarySelectionString;
	else
		selectionString = &s_Data.ClipboardString;
...
TRAP::INTERNAL::WindowingAPI::InternalVideoMode TRAP::INTERNAL::WindowingAPI::PlatformGetVideoModeX11(const InternalMonitor* monitor)
{
	...
	{
		XRRScreenResources* sr = s_Data.X11.RandR.GetScreenResourcesCurrent(s_Data.X11.display, s_Data.X11.Root);
		XRRCrtcInfo* ci = s_Data.X11.RandR.GetCrtcInfo(s_Data.X11.display, sr, monitor->CRTC);
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetWindowPosX11(const InternalWindow* window, const int32_t xPos, const int32_t yPos)
{
	...
	if(!PlatformWindowVisibleX11(window))
	{
		...
		if(s_Data.X11.XLIB.GetWMNormalHints(s_Data.X11.display, window->Handle, hints, &supplied))
		{
	...
	s_Data.X11.XLIB.MoveWindow(s_Data.X11.display, window->Handle, xPos, yPos);
															...
															const int32_t height, int32_t /*refreshRate*/)
{
	if(window->Monitor == monitor)
	{
		...
		else
		{
			...
			s_Data.X11.XLIB.MoveResizeWindow(s_Data.X11.display, window->Handle, xPos, yPos, width, height);
		}
	...
	if(window->Monitor)
	{
		if(!PlatformWindowVisibleX11(window))
		{
			s_Data.X11.XLIB.MapRaised(s_Data.X11.display, window->Handle);
...
std::vector<TRAP::INTERNAL::WindowingAPI::InternalVideoMode> TRAP::INTERNAL::WindowingAPI::PlatformGetVideoModesX11(const InternalMonitor* monitor)
{
	...
	if(s_Data.X11.RandR.Available && !s_Data.X11.RandR.MonitorBroken)
	{
		XRRScreenResources* sr = s_Data.X11.RandR.GetScreenResourcesCurrent(s_Data.X11.display, s_Data.X11.Root);
		XRRCrtcInfo* ci = s_Data.X11.RandR.GetCrtcInfo(s_Data.X11.display, sr, monitor->CRTC);
		XRROutputInfo* oi = s_Data.X11.RandR.GetOutputInfo(s_Data.X11.display, sr, monitor->Output);
...
bool TRAP::INTERNAL::WindowingAPI::PlatformInitX11()
{
	...
	if (s_Data.X11.XLIB.UTF8LookupString && s_Data.X11.XLIB.UTF8SetWMProperties)
...
void TRAP::INTERNAL::WindowingAPI::PlatformDestroyWindowX11(InternalWindow* window)
{
	...
	if(window->IC)
	{
		s_Data.X11.XLIB.DestroyIC(window->IC);
		window->IC = nullptr;
	}

	if(window->Handle)
	{
		s_Data.X11.XLIB.DeleteContext(s_Data.X11.display, window->Handle, s_Data.X11.Context);
		s_Data.X11.XLIB.UnmapWindow(s_Data.X11.display, window->Handle);
		s_Data.X11.XLIB.DestroyWindow(s_Data.X11.display, window->Handle);
		window->Handle = 0;
	}

	if(window->colormap)
	{
		s_Data.X11.XLIB.FreeColormap(s_Data.X11.display, window->colormap);
		window->colormap = 0;
	}
...
{
	if(s_Data.X11.HelperWindowHandle)
	{
		...
		s_Data.X11.HelperWindowHandle = 0;
	}

	if(s_Data.X11.HiddenCursorHandle)
	{
		s_Data.X11.XLIB.FreeCursor(s_Data.X11.display, s_Data.X11.HiddenCursorHandle);
		s_Data.X11.HiddenCursorHandle = 0;
	}
...
void TRAP::INTERNAL::WindowingAPI::PlatformShowWindowX11(InternalWindow* window)
{
	...
	s_Data.X11.XLIB.MapWindow(s_Data.X11.display, window->Handle);
	WaitForVisibilityNotify(window);
}
...
void TRAP::INTERNAL::WindowingAPI::PlatformFocusWindowX11(const InternalWindow* window)
{
	...
	{
		s_Data.X11.XLIB.RaiseWindow(s_Data.X11.display, window->Handle);
		s_Data.X11.XLIB.SetInputFocus(s_Data.X11.display, window->Handle, RevertToParent, CurrentTime);
	}
			                                            ...
			                                            WindowConfig& WNDConfig)
{
	...
	if(!visual)
	{
		visual = DefaultVisual(s_Data.X11.display, s_Data.X11.Screen);
		depth = DefaultDepth(s_Data.X11.display, s_Data.X11.Screen);
	}
...
bool TRAP::INTERNAL::WindowingAPI::PlatformCreateStandardCursorX11(InternalCursor* cursor, const CursorType& type)
{
	if(s_Data.X11.XCursor.Handle)
	{
		const char* theme = s_Data.X11.XCursor.GetTheme(s_Data.X11.display);
		if(theme)
		{
			...
			XcursorImage* image = s_Data.X11.XCursor.LibraryLoadImage(name.c_str(), theme, size);
			if(image)
			{
				cursor->Handle = s_Data.X11.XCursor.ImageLoadCursor(s_Data.X11.display, image);
	...
	if(!cursor->Handle)
	{
		...
		cursor->Handle = s_Data.X11.XLIB.CreateFontCursor(s_Data.X11.display, native);
		if(!cursor->Handle)
		{
...
{
	if(cursor->Handle)
		s_Data.X11.XLIB.FreeCursor(s_Data.X11.display, cursor->Handle);
}
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetCursorPosX11(InternalWindow* window, const double xPos, const double yPos)
{
	...
	s_Data.X11.XLIB.WarpPointer(s_Data.X11.display, None, window->Handle, 0, 0, 0, 0, static_cast<int32_t>(xPos),
...
void TRAP::INTERNAL::WindowingAPI::PlatformGetCursorPosX11(const InternalWindow* window, double& xPos, double& yPos)
{
	...
	s_Data.X11.XLIB.QueryPointer(s_Data.X11.display, window->Handle, &root, &child, &rootX, &rootY, &childX, &childY,
	                             &mask);
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetWindowIconX11(InternalWindow* window, const Image* const image)
{
	if(image)
	{
		...
		*target++ = image->GetHeight();
		...
		s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_ICON, XA_CARDINAL, 32,
		                               PropModeReplace,	reinterpret_cast<uint8_t*>(icon.data()),
								       static_cast<int32_t>(longCount));
	}
	else
		s_Data.X11.XLIB.DeleteProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_ICON);
...
void TRAP::INTERNAL::WindowingAPI::PlatformGetWindowPosX11(const InternalWindow* window, int32_t& xPos, int32_t& yPos)
{
	...
	s_Data.X11.XLIB.TranslateCoordinates(s_Data.X11.display, window->Handle, s_Data.X11.Root, 0, 0, &x, &y, &dummy);
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetWindowSizeX11(InternalWindow* window, const int32_t width, const int32_t height)
{
	...
	else
	{
		...
		s_Data.X11.XLIB.ResizeWindow(s_Data.X11.display, window->Handle, width, height);
	}
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetWindowDecoratedX11(const InternalWindow* window, const bool enabled)
{
	...
	s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.MOTIF_WM_HINTS, s_Data.X11.MOTIF_WM_HINTS, 32,
...
{
	if(!s_Data.X11.NET_WM_STATE || !s_Data.X11.NET_WM_STATE_ABOVE)
		return;
	...
	else
	{
		...
		const uint64_t count = GetWindowPropertyX11(window->Handle, s_Data.X11.NET_WM_STATE, XA_ATOM,
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetWindowOpacityX11(const InternalWindow* window, const float opacity)
{
	...
	s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_WINDOW_OPACITY, XA_CARDINAL, 32,
	                               PropModeReplace, reinterpret_cast<const uint8_t*>(&value), 1);
}
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetWindowMousePassthroughX11(InternalWindow* window, const bool enabled)
{
	...
	if (enabled)
	{
		...
		s_Data.X11.XShape.CombineRegion(s_Data.X11.display, window->Handle, ShapeInput, 0, 0, region, ShapeSet);
		s_Data.X11.XLIB.DestroyRegion(region);
	}
	else
		s_Data.X11.XShape.CombineMask(s_Data.X11.display, window->Handle, ShapeInput, 0, 0, None, ShapeSet);
}
...
float TRAP::INTERNAL::WindowingAPI::PlatformGetWindowOpacityX11(const InternalWindow* window)
{
	...
	if(s_Data.X11.XLIB.GetSelectionOwner(s_Data.X11.display, s_Data.X11.NET_WM_CM_Sx))
	{
		CARD32* value = nullptr;

		if(GetWindowPropertyX11(window->Handle, s_Data.X11.NET_WM_WINDOW_OPACITY, XA_CARDINAL,
                                                              ...
                                                              int32_t& yPos, int32_t& width, int32_t& height)
{
	...
	if(s_Data.X11.NET_WORKAREA && s_Data.X11.NET_CURRENT_DESKTOP)
	{
...
bool TRAP::INTERNAL::WindowingAPI::PlatformWindowVisibleX11(const InternalWindow* window)
{
	...
	s_Data.X11.XLIB.GetWindowAttributes(s_Data.X11.display, window->Handle, &wa);
...
const char* TRAP::INTERNAL::WindowingAPI::PlatformGetScanCodeNameX11(const int32_t scanCode)
{
	...
	const KeySym keySym = s_Data.X11.XKB.KeycodeToKeysym(s_Data.X11.display, scanCode,
																   ...
																   VkSurfaceKHR& surface)
{
	if(s_Data.VK.KHR_XCB_Surface && s_Data.X11.XCB.Handle)
	{
		xcb_connection_t* connection = s_Data.X11.XCB.GetXCBConnection(s_Data.X11.display);
		if(!connection)
		{
...
void TRAP::INTERNAL::WindowingAPI::PlatformMaximizeWindowX11(const InternalWindow* window)
{
	...
	else
	{
		...
		s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.NET_WM_STATE, XA_ATOM, 32, PropModeAppend,
...
void TRAP::INTERNAL::WindowingAPI::PlatformMinimizeWindowX11(const InternalWindow* window)
{
	...
	s_Data.X11.XLIB.IconifyWindow(s_Data.X11.display, window->Handle, s_Data.X11.Screen);
	s_Data.X11.XLIB.Flush(s_Data.X11.display);
}
...
void TRAP::INTERNAL::WindowingAPI::ProcessEvent(XEvent& event)
{
	...
	filtered = s_Data.X11.XLIB.FilterEvent(&event, 0);
	...
	if(event.type == GenericEvent)
	{
		if(s_Data.X11.XI.Available)
		{
			...
			if(window && window->RawMouseMotion && event.xcookie.extension == s_Data.X11.XI.MajorOPCode &&
			   s_Data.X11.XLIB.GetEventData(s_Data.X11.display, &event.xcookie) && event.xcookie.evtype == XI_RawMotion)
			{
	...
	case KeyPress:
	{
		...
		if(window->IC)
		{
			...
			if(!filtered)
			{
				...
				count = s_Data.X11.XLIB.UTF8LookupString(window->IC, &event.xkey, buffer.data(),
				...
				if(status == XBufferOverflow)
				{
					...
					chars = buffer.data();
					count = s_Data.X11.XLIB.UTF8LookupString(window->IC, &event.xkey, chars, count, nullptr, &status);
				}
	...
	case KeyRelease:
	{
		...
		if(!s_Data.X11.XKB.Detectable)
		{
			...
			if(s_Data.X11.XLIB.EventsQueued(s_Data.X11.display, QueuedAfterReading))
			{
				XEvent next;
	...
	case ConfigureNotify:
	{
		...
		if(!event.xany.send_event && window->Parent != s_Data.X11.Root)
		{
			GrabErrorHandlerX11();

			::Window dummy = 0;
			s_Data.X11.XLIB.TranslateCoordinates(s_Data.X11.display, window->Parent, s_Data.X11.Root, xPos, yPos, &xPos, &yPos, &dummy);
	...
	case ClientMessage:
	{
		...
		else if(event.xclient.message_type == s_Data.X11.XDNDEnter)
		{
			...
			s_Data.X11.XDND.Version = static_cast<int32_t>(event.xclient.data.l[1] >> 24);
		...
		else if(event.xclient.message_type == s_Data.X11.XDNDDrop)
		{
			...
			if(s_Data.X11.XDND.Format)
			{
				...
				s_Data.X11.XLIB.ConvertSelection(s_Data.X11.display, s_Data.X11.XDNDSelection, s_Data.X11.XDND.Format,
				                                 s_Data.X11.XDNDSelection, window->Handle, time);
			}
		...
		else if(event.xclient.message_type == s_Data.X11.XDNDPosition)
		{
			...
			s_Data.X11.XLIB.TranslateCoordinates(s_Data.X11.display, s_Data.X11.Root, window->Handle, xAbs, yAbs, &xPos, &yPos,
			                                     &dummy);
	...
	case FocusIn:
	{
		...
		if(window->IC)
			s_Data.X11.XLIB.SetICFocus(window->IC);
	...
	case FocusOut:
	{
		...
		if(window->IC)
			s_Data.X11.XLIB.UnsetICFocus(window->IC);
...
void TRAP::INTERNAL::WindowingAPI::AcquireMonitor(InternalWindow* window)
{
	...
	if(window->OverrideRedirect)
	{
		...
		s_Data.X11.XLIB.MoveResizeWindow(s_Data.X11.display, window->Handle, xPos, yPos, mode.Width, mode.Height);
	}
...
void TRAP::INTERNAL::WindowingAPI::SetVideoModeX11(InternalMonitor* monitor, const InternalVideoMode& desired)
{
	...
	if(native)
	{
		...
		s_Data.X11.RandR.SetCrtcConfig(s_Data.X11.display, sr, monitor->CRTC, CurrentTime, ci->x, ci->y, native,
...
void TRAP::INTERNAL::WindowingAPI::RestoreVideoModeX11(InternalMonitor* monitor)
{
	...
	s_Data.X11.RandR.SetCrtcConfig(s_Data.X11.display, sr, monitor->CRTC, CurrentTime, ci->x, ci->y, monitor->OldMode,
	                              ci->rotation, ci->outputs, ci->noutput);
...
void TRAP::INTERNAL::WindowingAPI::UpdateCursorImage(const InternalWindow* window)
{
	...
	{
		if(window->Cursor)
			s_Data.X11.XLIB.DefineCursor(s_Data.X11.display, window->Handle, window->Cursor->Handle);
		else
			s_Data.X11.XLIB.UndefineCursor(s_Data.X11.display, window->Handle);
	}
	else
		s_Data.X11.XLIB.DefineCursor(s_Data.X11.display, window->Handle, s_Data.X11.HiddenCursorHandle);
}
...
void TRAP::INTERNAL::WindowingAPI::CreateKeyTables()
{
	...
	int32_t width = 0;
	KeySym* keySyms = s_Data.X11.XLIB.GetKeyboardMapping(s_Data.X11.display, scanCodeMin, scanCodeMax - scanCodeMin + 1,
	                                                     &width);
...
{
	s_Data.X11.XLIB.GrabPointer(s_Data.X11.display, window->Handle, 1,
...
void TRAP::INTERNAL::WindowingAPI::PlatformSetDragAndDropX11(InternalWindow* window, const bool value)
{
	if(value)
	{
		...
		s_Data.X11.XLIB.ChangeProperty(s_Data.X11.display, window->Handle, s_Data.X11.XDNDAware, XA_ATOM, 32,
			                       PropModeReplace, reinterpret_cast<const uint8_t*>(&version), 1);
	}
	else
		s_Data.X11.XLIB.DeleteProperty(s_Data.X11.display, window->Handle, s_Data.X11.XDNDAware);
}
