...
{
	OpenGLShader::OpenGLShader(std::string name, std::string VSSource, std::string FSSource, std::string GSSource, std::string TCSSource, std::string TESSource, std::string CSSource)
	...
	void OpenGLShader::Shutdown() const
	{
	...
	unsigned int OpenGLShader::Compile(std::array<std::string*, 6>& shaders, OpenGLShaderErrorInfo& info)
	{
	...
	void OpenGLShader::Parse(const std::string& VSSource, const std::string& FSSource, const std::string& GSSource, const std::string& TCSSource, const std::string& TESSource, const std::string& CSSource)
	{
		...
		//Geometry Shader
		gstr = GSSource.c_str();
		while ((token = Utils::FindToken(gstr, "struct")))
			ParseUniformStruct(Utils::GetBlock(token, &gstr), 2);

		gstr = GSSource.c_str();
		while ((token = Utils::FindToken(gstr, "uniform")))
			ParseUniform(Utils::GetStatement(token, &gstr), 2);

		//Tessellation Control Shader
		tcstr = TCSSource.c_str();
		while ((token = Utils::FindToken(tcstr, "struct")))
			ParseUniformStruct(Utils::GetBlock(token, &tcstr), 3);

		tcstr = TCSSource.c_str();
		while ((token = Utils::FindToken(tcstr, "uniform")))
			ParseUniform(Utils::GetStatement(token, &tcstr), 3);

		//Tessellation Evaluation Shader
		testr = TESSource.c_str();
		while ((token = Utils::FindToken(testr, "struct")))
			ParseUniformStruct(Utils::GetBlock(token, &testr), 4);

		testr = TESSource.c_str();
		while ((token = Utils::FindToken(testr, "uniform")))
			ParseUniform(Utils::GetStatement(token, &testr), 4);

		//Compute Shader
		cstr = CSSource.c_str();
		while ((token = Utils::FindToken(cstr, "struct")))
			ParseUniformStruct(Utils::GetBlock(token, &cstr), 5);

		cstr = CSSource.c_str();
		while ((token = Utils::FindToken(cstr, "uniform")))
			ParseUniform(Utils::GetStatement(token, &cstr), 5);
	}

	void OpenGLShader::ParseUniform(const std::string& statement, const unsigned int shaderType)
	{
	...
	void OpenGLShader::ParseUniformStruct(const std::string& block, unsigned int shaderType)
	{
	...
	bool OpenGLShader::IsSystemUniform(ShaderUniformDeclaration* uniform)
	{
	...
	void OpenGLShader::SetUniformStruct(OpenGLShaderUniformDeclaration* uniform, uint8_t* data, int32_t offset) const
	{
	...
	ShaderUniformDeclaration* OpenGLShader::FindUniformDeclaration(const std::string& name, ShaderUniformBufferDeclaration* buffer)
	{
	...
	void OpenGLShader::SetVSSystemUniformBuffer(uint8_t* data, const unsigned int size, const unsigned int slot)
	{
		Bind();
	...
	void OpenGLShader::SetFSSystemUniformBuffer(uint8_t* data, const unsigned int size, const unsigned int slot)
	{		
		Bind();
	...
	void OpenGLShader::SetGSSystemUniformBuffer(uint8_t* data, const unsigned int size, const unsigned int slot)
	{
		Bind();
	...
	void OpenGLShader::SetTCSSystemUniformBuffer(uint8_t* data, const unsigned int size, const unsigned int slot)
	{
		Bind();
	...
	void OpenGLShader::SetTESSystemUniformBuffer(uint8_t* data, const unsigned int size, const unsigned int slot)
	{
		Bind();
	...
	void OpenGLShader::SetCSSystemUniformBuffer(uint8_t* data, const unsigned int size, const unsigned int slot)
	{
		Bind();
	...
	void OpenGLShader::SetVSUserUniformBuffer(uint8_t* data, const unsigned int size)
	{
		ResolveAndSetUniforms(m_VSUserUniformBuffer.get(), data, size);
	}

	void OpenGLShader::SetFSUserUniformBuffer(uint8_t* data, const unsigned int size)
	{
		ResolveAndSetUniforms(m_FSUserUniformBuffer.get(), data, size);
	}

	void OpenGLShader::SetGSUserUniformBuffer(uint8_t* data, const unsigned int size)
	{
		ResolveAndSetUniforms(m_GSUserUniformBuffer.get(), data, size);
	}

	void OpenGLShader::SetTCSUserUniformBuffer(uint8_t* data, const unsigned int size)
	{
		ResolveAndSetUniforms(m_TCSUserUniformBuffer.get(), data, size);
	}

	void OpenGLShader::SetTESUserUniformBuffer(uint8_t* data, const unsigned int size)
	{
		ResolveAndSetUniforms(m_TESUserUniformBuffer.get(), data, size);
	}

	void OpenGLShader::SetCSUserUniformBuffer(uint8_t* data, const unsigned int size)
	{
		ResolveAndSetUniforms(m_CSUserUniformBuffer.get(), data, size);
	}

	ShaderStruct* OpenGLShader::FindStruct(const std::string_view name)
	{
	...
	void OpenGLShader::ResolveAndSetUniforms(ShaderUniformBufferDeclaration* buffer, uint8_t* data, const unsigned int size) const
	{
		const ShaderUniformList& uniforms = buffer->GetUniformDeclarations();
	...
	void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, uint8_t* data, unsigned int size) const
	{
	...
	void OpenGLShader::SetUniform(const std::string& name, uint8_t* data)
	{
	...
	void OpenGLShader::ResolveAndSetUniformField(const OpenGLShaderUniformDeclaration& field, uint8_t* data, const int32_t offset) const
	{
	...
	void OpenGLShader::SetUniform1fv(const std::string& name, float* value, const int32_t count) const
	{
		SetUniform1fv(GetUniformLocation(name), value, count);
	}

	void OpenGLShader::SetUniform1i(const std::string& name, const int32_t value) const
	{
		SetUniform1i(GetUniformLocation(name), value);
	}

	void OpenGLShader::SetUniform1iv(const std::string& name, int32_t* value, const int32_t count) const
	{
		SetUniform1iv(GetUniformLocation(name), value, count);
	}

	void OpenGLShader::SetUniform2f(const std::string& name, const Maths::Vec2& vector) const
	{
		SetUniform2f(GetUniformLocation(name), vector);
	}

	void OpenGLShader::SetUniform3f(const std::string& name, const Maths::Vec3& vector) const
	{
		SetUniform3f(GetUniformLocation(name), vector);
	}

	void OpenGLShader::SetUniform4f(const std::string& name, const Maths::Vec4& vector) const
	{
		SetUniform4f(GetUniformLocation(name), vector);
	}

	void OpenGLShader::SetUniformMat3(const std::string& name, const Maths::Mat3& matrix) const
	{
		SetUniformMat3(GetUniformLocation(name), matrix);
	}

	void OpenGLShader::SetUniformMat4(const std::string& name, const Maths::Mat4& matrix) const
	{
		SetUniformMat4(GetUniformLocation(name), matrix);
	}

	void OpenGLShader::SetUniform1f(const unsigned int location, const float value)
	{
		OpenGLCall(glUniform1f(location, value));
	}

	void OpenGLShader::SetUniform1fv(const unsigned int location, float* value, const int32_t count)
	{
		OpenGLCall(glUniform1fv(location, count, value));
	}

	void OpenGLShader::SetUniform1i(const unsigned int location, const int32_t value)
	{
		OpenGLCall(glUniform1i(location, value));
	}

	void OpenGLShader::SetUniform1iv(const unsigned int location, int32_t* value, const int32_t count)
	{
		OpenGLCall(glUniform1iv(location, count, value));
	}

	void OpenGLShader::SetUniform2f(const unsigned int location, const Maths::Vec2& vector)
	{
		OpenGLCall(glUniform2f(location, vector.x, vector.y));
	}

	void OpenGLShader::SetUniform3f(const unsigned int location, const Maths::Vec3& vector)
	{
		OpenGLCall(glUniform3f(location, vector.x, vector.y, vector.z));
	}

	void OpenGLShader::SetUniform4f(const unsigned int location, const Maths::Vec4& vector)
	{
		OpenGLCall(glUniform4f(location, vector.x, vector.y, vector.z, vector.w));
	}

	void OpenGLShader::SetUniformMat3(const unsigned int location, const Maths::Mat3& matrix)
	{
		OpenGLCall(glUniformMatrix3fv(location, 1, GL_TRUE, matrix.elements.data()));
	}

	void OpenGLShader::SetUniformMat4(const unsigned int location, const Maths::Mat4& matrix)
	{
	...
	const std::string& OpenGLShader::GetVSFilePath() const
	{
		return m_VSFilepath;
	}

	const std::string& OpenGLShader::GetFSFilePath() const
	{
		return m_FSFilepath;
	}

	const std::string& OpenGLShader::GetGSFilePath() const
	{
		return m_GSFilepath;
	}

	const std::string& OpenGLShader::GetTCSFilePath() const
	{
		return m_TCSFilepath;
	}

	const std::string& OpenGLShader::GetTESFilePath() const
	{
		return m_TESFilepath;
	}

	const std::string& OpenGLShader::GetCSFilePath() const
	{
		return m_CSFilepath;
	}

	const std::string& OpenGLShader::GetVSSource() const
	{
		return m_VSSource;
	}

	const std::string& OpenGLShader::GetFSSource() const
	{
		return m_FSSource;
	}

	const std::string& OpenGLShader::GetGSSource() const
	{
		return m_GSSource;
	}

	const std::string& OpenGLShader::GetTCSSource() const
	{
		return m_TCSSource;
	}

	const std::string& OpenGLShader::GetTESSource() const
	{
		return m_TESSource;
	}

	const std::string& OpenGLShader::GetCSSource() const
	{
		return m_CSSource;
	}

	const ShaderUniformBufferList& OpenGLShader::GetVSSystemUniforms() const
	{
		return m_VSUniformBuffers;
	}

	const ShaderUniformBufferList& OpenGLShader::GetFSSystemUniforms() const
	{
		return m_FSUniformBuffers;
	}

	const ShaderUniformBufferList& OpenGLShader::GetGSSystemUniforms() const
	{
		return m_GSUniformBuffers;
	}

	const ShaderUniformBufferList& OpenGLShader::GetTCSSystemUniforms() const
	{
		return m_TCSUniformBuffers;
	}

	const ShaderUniformBufferList& OpenGLShader::GetTESSystemUniforms() const
	{
		return m_TESUniformBuffers;
	}

	const ShaderUniformBufferList& OpenGLShader::GetCSSystemUniforms() const
	{
		return m_CSUniformBuffers;
	}

	ShaderUniformBufferDeclaration* OpenGLShader::GetVSUserUniformBuffer() const
	{
		return m_VSUserUniformBuffer.get();
	}

	ShaderUniformBufferDeclaration* OpenGLShader::GetFSUserUniformBuffer() const
	{
		return m_FSUserUniformBuffer.get();
	}

	ShaderUniformBufferDeclaration* OpenGLShader::GetGSUserUniformBuffer() const
	{
		return m_GSUserUniformBuffer.get();
	}

	ShaderUniformBufferDeclaration* OpenGLShader::GetTCSUserUniformBuffer() const
	{
		return m_TCSUserUniformBuffer.get();
	}

	ShaderUniformBufferDeclaration* OpenGLShader::GetTESUserUniformBuffer() const
	{
		return m_TESUserUniformBuffer.get();
	}

	ShaderUniformBufferDeclaration* OpenGLShader::GetCSUserUniformBuffer() const
	{
		return m_CSUserUniformBuffer.get();
	}

	const ShaderResourceList& OpenGLShader::GetResource() const
	{
		return m_resources;
	}
}