...
{
	TRAP_DEBUG("[Context][Vulkan] Destroying Context");
}
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Initializing Surface");
...
void TRAP::Graphics::API::VulkanContext::DeInitSurface(VkInstance instance)
{
	if (m_surface)
	{
		TRAP_DEBUG("[Renderer][Vulkan] Destroying Surface");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Setting up Swapchain");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Initializing Swapchain");
...
void TRAP::Graphics::API::VulkanContext::DeInitSwapchain(VkDevice device)
{
	if (m_swapchain)
	{
		TRAP_DEBUG("[Renderer][Vulkan] Destroying Swapchain");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Initializing Swapchain Image Views");
...
void TRAP::Graphics::API::VulkanContext::DeInitImageViews(VkDevice device)
{
	if (!m_swapchainImageViews.empty())
		TRAP_DEBUG("[Renderer][Vulkan] Destroying Swapchain Image Views");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Getting available Surface Formats");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Getting available Surface Present Modes");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Choosing Swapchain Surface Format");

	if (availableSurfaceFormats.size() == 1 && availableSurfaceFormats[0].format == VK_FORMAT_UNDEFINED)
	{
		TRAP_DEBUG("[Renderer][Vulkan] Using Surface Format: VK_FORMAT_B8G8R8A8_UNORM, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR");
	...
	TRAP_DEBUG("[Renderer][Vulkan] Using Surface Format: Physical Device Default Format, Physical Device Default Color Space");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Choosing Swapchain Surface Present Mode");
	...
	for (const auto& availableSurfacePresentMode : availableSurfacePresentModes)
	{
		...
		{
			TRAP_DEBUG("[Renderer][Vulkan] Using Surface Present Mode: VK_PRESENT_MODE_MAILBOX_KHR(Triple Buffered)");
			return availableSurfacePresentMode;
		}
		if (availableSurfacePresentMode == VK_PRESENT_MODE_IMMEDIATE_KHR)
		{
			TRAP_DEBUG("[Renderer][Vulkan] Using Surface Present Mode: VK_PRESENT_MODE_IMMEDIATE_KHR");
			return availableSurfacePresentMode; //Immediate Swaps High Tearing(Preferred over VSync because of driver incompatibilities)
		}
	}

	TRAP_DEBUG("[Renderer][Vulkan] Using Surface Present Mode: VK_PRESENT_MOE_FIFO_KHR(VSync Double Buffered");
...
{
	TRAP_DEBUG("[Renderer][Vulkan] Choosing Swapchain Extent");

	if (m_capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max())
	{
		TRAP_DEBUG("[Renderer][Vulkan] Using Swapchain Extent: ", m_capabilities.currentExtent.width, 'x', m_capabilities.currentExtent.height);
	...
	TRAP_DEBUG("[Renderer][Vulkan] Using Swapchain Extent: ", actualExtent.width, 'x', actualExtent.height);
