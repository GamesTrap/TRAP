...
void TRAP::Graphics::API::VulkanTexture::SetTextureName(const std::string_view name) const
{
	...
	VkSetObjectName(m_device->GetVkDevice(), Utils::BitCast<uint64_t>(m_vkImage), VK_OBJECT_TYPE_IMAGE, name);
#else
	VkSetObjectName(m_device->GetVkDevice(), Utils::BitCast<uint64_t>(m_vkImage), VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, name);
#endif
}
