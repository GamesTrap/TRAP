...
#define BIND_EVENT_FN(x) std::bind(&Application::x, this, std::placeholders::_1)
		...
		: m_timer(std::make_unique<Utils::Timer>()), m_FramesPerSecond(0), m_Frametime(0.0f)
	{
		...
		m_window = std::make_unique<Window>(WindowProps("Sandbox", width, height, refreshRate, vsync, displayMode, monitor, renderAPI));
	...
	void Application::Run()
	{
		...
		while (m_running)
		{
			...
			{
				//Start monitoring shader folders for changes and(in case of changes) run a ShaderManager::Reload(virtualPath)
				VFS::Get()->GetShaderFileWatcher()->Start([](const std::filesystem::path& physicalPath, const std::string& virtualPath, const FileStatus status) -> void
					{
