...
namespace
{
	...
	[[nodiscard]] bool IHDRCheck(const IHDRChunk& ihdrChunk)
	{
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Width ", ihdrChunk.Width, " is invalid!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Height ", ihdrChunk.Height, " is invalid!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Bit depth ", ihdrChunk.BitDepth, " is invalid!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Color type ", ihdrChunk.ColorType, " is invalid!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Color type: TrueColor(", NumericCast<u32>(ihdrChunk.ColorType),
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Color type: Indexed-Color(", NumericCast<u32>(ihdrChunk.ColorType),
				") doesnt allow a bit depth of ", NumericCast<u32>(ihdrChunk.BitDepth), "!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Color type: GrayScale Alpha(", NumericCast<u32>(ihdrChunk.ColorType),
				") doesnt allow a bit depth of ", NumericCast<u32>(ihdrChunk.BitDepth), "!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Color type: TrueColor Alpha(", NumericCast<u32>(ihdrChunk.ColorType),
				") doesnt allow a bit depth of ", NumericCast<u32>(ihdrChunk.BitDepth), "!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Compression method: ", NumericCast<u32>(ihdrChunk.CompressionMethod),
				" is unsupported!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Filter method: ", NumericCast<u32>(ihdrChunk.CompressionMethod),
				" is unsupported!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Interlace method: ", NumericCast<u32>(ihdrChunk.InterlaceMethod),
				" is unsupported!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
	...
	[[nodiscard]] bool ProcessIHDR(std::ifstream& file, Data& data)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None &&
												 (GetTRAPProfileSystems() & ProfileSystems::Verbose) != ProfileSystems::None);
		...
		{
			TRAP::Utils::Memory::SwapBytes(ihdrChunk.Width);
			TRAP::Utils::Memory::SwapBytes(ihdrChunk.Height);
		}

		const std::array<u8, 17> CRCData
		{
			'I', 'H', 'D', 'R',
			TRAP::Utils::Memory::GetByteFromInteger<3>(ihdrChunk.Width),
			TRAP::Utils::Memory::GetByteFromInteger<2>(ihdrChunk.Width),
			TRAP::Utils::Memory::GetByteFromInteger<1>(ihdrChunk.Width),
			TRAP::Utils::Memory::GetByteFromInteger<0>(ihdrChunk.Width),
			TRAP::Utils::Memory::GetByteFromInteger<3>(ihdrChunk.Height),
			TRAP::Utils::Memory::GetByteFromInteger<2>(ihdrChunk.Height),
			TRAP::Utils::Memory::GetByteFromInteger<1>(ihdrChunk.Height),
			TRAP::Utils::Memory::GetByteFromInteger<0>(ihdrChunk.Height),
			ihdrChunk.BitDepth, ihdrChunk.ColorType, ihdrChunk.CompressionMethod, ihdrChunk.FilterMethod,
			ihdrChunk.InterlaceMethod
		};

		const std::array<u8, 4> crc = TRAP::Utils::Hash::CRC32(CRCData.data(), CRCData.size());
		if(crc != ihdrChunk.CRC)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "IHDR CRC: ", TRAP::Utils::Hash::ConvertHashToString(ihdrChunk.CRC), " is wrong!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
	...
	/// @param data Data containing information about the image.
	/// @return True if the chunk was processed successfully, false otherwise.
	[[nodiscard]] bool ProcesssBIT(std::ifstream& file, const Data& data)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None &&
												 (GetTRAPProfileSystems() & ProfileSystems::Verbose) != ProfileSystems::None);
	...
	/// @param file Open PNG file.
	/// @return True if the chunk was processed successfully, false otherwise.
	[[nodiscard]] bool ProcesssRGB(std::ifstream& file)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None &&
												 (GetTRAPProfileSystems() & ProfileSystems::Verbose) != ProfileSystems::None);
		...
		if(CRC != crc)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "sRGB CRC: ", TRAP::Utils::Hash::ConvertHashToString(CRC), " is wrong!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
	...
	/// @param data Data containing information about the image.
	/// @return True if the chunk was processed successfully, false otherwise.
	[[nodiscard]] bool ProcessbKGD(std::ifstream& file, const Data& data)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None &&
												 (GetTRAPProfileSystems() & ProfileSystems::Verbose) != ProfileSystems::None);
	...
	/// @param data Data containing information about the image.
	/// @return True if the chunk was processed successfully, false otherwise.
	[[nodiscard]] bool ProcesstRNS(std::ifstream& file, const u32 length, Data& data)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None &&
												 (GetTRAPProfileSystems() & ProfileSystems::Verbose) != ProfileSystems::None);
		...
		case 0:
		{
			...
			if(crc != CRC)
			{
				TP_ERROR(TRAP::Log::ImagePNGPrefix, "tRNS CRC: ", TRAP::Utils::Hash::ConvertHashToString(CRC), " is wrong!");
				TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
				return false;
			}
	...
	/// @param length Chunk length.
	/// @return True if the chunk was processed successfully, false otherwise.
	[[nodiscard]] bool ProcessPLTE(std::ifstream& file, Data& data, const u32 length)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None);

		if (length % 3 != 0)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "PLTE length ", length, " is not divisible by 3!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		if (data.ColorType == 0 || data.ColorType == 4)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "PLTE invalid usage! This chunk should not appear with color type: ",
		...
		if(crc != CRC)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "PLTE CRC: ", TRAP::Utils::Hash::ConvertHashToString(CRC), " is wrong!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
	...
	/// @param length Chunk length.
	/// @return True if the chunk was processed successfully, false otherwise.
	[[nodiscard]] bool ProcessIDAT(std::ifstream& file, Data& data, const u32 length)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None &&
												 (GetTRAPProfileSystems() & ProfileSystems::Verbose) != ProfileSystems::None);
		...
		if(crc != CRC)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "IDAT CRC: ", TRAP::Utils::Hash::ConvertHashToString(CRC), " is wrong!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
	...
	[[nodiscard]] bool DecompressData(const std::span<const u8> source, const std::span<u8> destination)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None);

		if (source.size_bytes() < 2)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Compressed zlib data is too small!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false; //Error, size of zlib data too small
		}
		if ((source[0] * 256 + source[1]) % 31 != 0)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Decompression failed! 256 * source[0](", source[0], ") + source[1](",
		...
		if (CM != 8 || CINFO > 7)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Decompression failed! Only compression method 8(De/Inflate) with ",
		...
		if(FDICT != 0)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Decompression failed! Additional flags should ",
		...
		if (!TRAP::Utils::Decompress::Inflate(source.subspan<2>(), destination))
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Decompression failed! Inflate error!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
			return false;
		}
		...
		const std::array<u8, 4> checksum = TRAP::Utils::Hash::Adler32(destination.data(), destination.size_bytes());

		if(checksum != adler32)
		{
			TP_ERROR(TRAP::Log::ImagePNGPrefix, "Decompression failed! Adler32 checksum: ",
										        TRAP::Utils::Hash::ConvertHashToString(adler32),
										        " doesnt match checksum: ",
												TRAP::Utils::Hash::ConvertHashToString(checksum), "!");
			TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
	...
	[[nodiscard]] constexpr u8 PaethPredictor(u16 a, const u16 b, const u16 c)
	{
		u16 pa = NumericCast<u16>(TRAP::Math::Abs(b - c));
		const u16 pb = NumericCast<u16>(TRAP::Math::Abs(a - c));
		const u16 pc = NumericCast<u16>(TRAP::Math::Abs(a + b - c - c));
	...
	[[nodiscard]] constexpr bool UnFilterScanline(u8* const recon, const u8* const scanline, const u8* const precon,
										          const usize byteWidth, const u8 filterType, const usize length)
	{
	...
	[[nodiscard]] constexpr bool UnFilter(u8* const out, const u8* const in, const u32 width, const u32 height,
	...
	[[nodiscard]] constexpr usize GetRawSizeIDAT(const u32 width, const u32 height, const u32 bitsPerPixel)
	{
		...
		const usize line = (NumericCast<usize>(width / 8u) * bitsPerPixel) + 1u + ((width & 7u) * bitsPerPixel + 7u) / 8u;
	...
	[[nodiscard]] constexpr usize GetRawSize(const u32 width, const u32 height, const u32 bitsPerPixel)
	{
	...
	/// @param bitsPerPixel Image bits per pixel.
	constexpr void Adam7GetPassValues(std::array<u32, 7>& passW, std::array<u32, 7>& passH,
	                                  std::array<usize, 8>& filterPassStart, std::array<usize, 8>& paddedPassStart,
							          std::array<usize, 8>& passStart, const u32 width, const u32 height,
							          const u32 bitsPerPixel)
	{
	...
	/// @param height Image height.
	/// @param bitsPerPixel Image bits per pixel.
	constexpr void Adam7DeInterlace(u8* const out, const u8* const in, const u32 width, const u32 height, const u32 bitsPerPixel)
	{
	...
	[[nodiscard]] constexpr bool PostProcessScanlines(u8* const out, u8* const in, const u32 width, const u32 height,
	                                                  const u32 bitsPerPixel, const u8 interlaceMethod)
	{
	...
	[[nodiscard]] constexpr std::vector<u16> ConvertTo2Byte(std::vector<u8>& raw)
	{
	...
	[[nodiscard]] constexpr std::vector<u8> ResolveIndexed(const std::vector<u8>& raw, const u32 width, const u32 height,
	                                                       const Data& data)
	{
	...
	/// @return True if the chunk was processed successfully, false otherwise.
	[[nodiscard]] bool ProcessChunk(NextChunk& nextChunk, std::ifstream& file, Data& data, AlreadyLoaded& alreadyLoaded)
	{
		ZoneNamedC(__tracy, tracy::Color::Green, (GetTRAPProfileSystems() & ProfileSystems::ImageLoader) != ProfileSystems::None);
		...
		TP_ERROR(TRAP::Log::ImagePNGPrefix, "Invalid or multiple usage of chunk magic number: ", nextChunk.MagicNumber, "!");
		TP_WARN(TRAP::Log::ImagePNGPrefix, "Using default image!");
		return false;
	}
}
