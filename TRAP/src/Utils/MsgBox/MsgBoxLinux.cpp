...
{
	GtkMessageType GetMessageType(const Style style)
	{
		switch(style)
		{
		case Style::Info:
	...
	GtkButtonsType GetButtonsType(const Buttons buttons)
	{
		switch(buttons)
		{
		case Buttons::OK:
														 ...
														 const Buttons buttons)
{
	...
	GtkWidget* dialog = gtk_message_dialog_new(GTK_WINDOW(parent),
	                                           GTK_DIALOG_MODAL,
	                                           INTERNAL::GetMessageType(style),
	                                           INTERNAL::GetButtonsType(buttons),
	                                           "%s",
	                                           message);
	gtk_window_set_title(GTK_WINDOW(dialog), title);
	const Selection selection = INTERNAL::GetSelection(gtk_dialog_run(GTK_DIALOG(dialog)));
